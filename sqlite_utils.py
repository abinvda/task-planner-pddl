...
def add_node(conn, fringe):
    ...
    sql = ''' INSERT INTO fringe(f_value,cost,state,level_id)
              VALUES(?,?,?,?) '''
    cur = conn.cursor()
...
def delete_node(conn, id):
    ...
    :return:
    """
    sql = 'DELETE FROM fringe WHERE id=?'
    cur = conn.cursor()
    cur.execute(sql, (id,))


def pop_node_by_priority(conn):
    """
    ...
    rows = cur.fetchall()
    row = min(rows, key=lambda x: x[2])
    delete_node(conn, row[0])
...
def main():
    ...
    sql_create_tasks_table = """CREATE TABLE IF NOT EXISTS fringe (
                                        ...
                                        FOREIGN KEY (level_name) REFERENCES game_levels (name)
                                    );"""
    ...
    with conn:
        ...
        print("1. Query task by priority:")
        node = pop_node_by_priority(conn)
